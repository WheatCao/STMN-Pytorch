...
import time
import datetime
from utils import SSIMLoss, StructuralSimilarityLoss
...
parser.add_argument('--num_workers', type=int, default=32, help='how many subprocesses to use for data loading')
...
def train_one_step(net, data, label, optimizer, criterion,epoch,clean_label):
    net.train()
    if epoch == 11 and label.shape[0]==128:
        pred = net(data,save=True)
        with open('./' + 'results_ours' + '/noise_label.txt', "ab") as nlf:
            np.savetxt(nlf, label.detach().cpu().numpy())
        with open('./' + 'results_ours' + '/clean_label.txt', "ab") as clf:
            np.savetxt(clf, clean_label.detach().cpu().numpy())
        # np.savetxt('./' + 'results_ours' + '/label.txt', label.detach().cpu().numpy())
    else:
        pred = net(data)
...
def train(train_loader, epoch, model1, optimizer1):
    ...
    for i, (data, noisy_label, clean_label, indexes) in enumerate(train_loader):
        ...
        prec1, loss = train_one_step(model1, data, labels, optimizer1, nn.CrossEntropyLoss(),epoch,clean_label)
...
def train_forward(model, train_loader, epoch, optimizer, Bayesian_T, revision=True):
    ...
    for i, (data, labels, clean_label, indexes) in enumerate(train_loader):
        ...
        if epoch == 21 and labels.shape[0]==128:
            logits, delta = model(data, revision=True,save=True)
            with open('./' + 'results_ours' + '/noise_label.txt', "ab") as nlf:
                np.savetxt(nlf, labels.detach().cpu().numpy())
            with open('./' + 'results_ours' + '/clean_label.txt', "ab") as clf:
                np.savetxt(clf, clean_label.detach().cpu().numpy())
            # np.savetxt('./' + 'results_ours' + '/label.txt', labels.detach().cpu().numpy())
        else:
        ...
        # Mixup loss.
        loss = lam * criterion(log_noisy_post.cuda(),labels.cuda()) + (1 - lam) * criterion(log_noisy_post.cuda(), labels[index].cuda())
...
def gen_T_mask(class_num,batch_size):
    T_mask = torch.zeros((class_num, class_num))
    T_mask[0][0] = 1
    T_mask[0][1] = 1
    for i in range(1,class_num-1):
        ...
        T_mask[i][i] = 1
        T_mask[i][i + 1] = 1
    T_mask[class_num-1][class_num-1] = 1
    T_mask[class_num-1][class_num-2] = 1
    T_mask = T_mask.unsqueeze(0)
    T_mask = T_mask.repeat(batch_size,1,1)
    return T_mask
...
#     # Define models
    ...
    for i, (data, noisy_label, clean_label, indexes) in enumerate((train_loader_batch_1)):
        ...
        distilled_example_index_list.extend(indexes.cpu()[mask.cpu()])
...
#     clf_bayes_output -> transition matrix with size c*c
    ...
    # optimizer_D = torch.optim.SGD(D_Network.parameters(), lr=0.03, momentum=0.9)
    optimizer_D = torch.optim.Adam(D_Network.parameters(), lr=0.007)
    loss_function1 = nn.NLLLoss()
    loss_function2 = nn.MSELoss()#L1Loss()
    ...
    for epoch in range(0, 50):
        ...
        for data, bayes_labels, noisy_labels, index in train_loader_distilled:
            ...
            mask_matrix_bs = mask_matrix.repeat(batch_matrix.size(0), 1, 1)
            T_mask = D_Network(mask_matrix_bs.cuda())
            S_mask = D_Network(batch_matrix.detach())
            ...
            loss1 = loss_function1(noisy_class_post.cuda(), noisy_labels) + loss_function2(S_mask,torch.zeros(S_mask.size()).cuda())
            loss2 = loss_function2(S_mask.mean(0).detach() ,T_mask.mean(0).detach())
            ...
            optimizer_D.step()
            bayes_loss += loss1.item()
        print('Bayesian-T Training Epoch [%d], Loss_G: %.4f ,Loss_D: %.4f'% (epoch + 1, loss1.item(),loss2.item()))
...
if __name__ == '__main__':
    ...
    for i in range(args.n):
        ...
        acc = main(args)
        acclist.append(acc)
        print("Time of epoch{}: {}".format(i+1, format_time(time.time() - start_time)))
    print(np.array(acclist).mean())
    print(np.array(acclist).std(ddof=1))
    acc_mean = np.array(acclist).mean()
    acc_std = np.array(acclist).std(ddof=1)
    accarr = np.array(acclist)
    accarr = np.append(accarr, acc_mean)
    accarr = np.append(accarr, acc_std)
    np.savetxt('./'+save_dir+'/acclist', accarr)
