...
class ResNet(nn.Module):
    ...
    def forward(self, x, revision=False,save=False):
        ...
        if save and out_1.size(0)==128:
            with open('./' + 'results_ours' + '/out_1.txt',"ab") as f:
                np.savetxt(f, out_1.detach().cpu().numpy())
            # np.savetxt('./' + 'results_ours' + '/out_1.txt', out_1.detach().cpu().numpy())
        out_2 = self.linear(out_1)
        
        if revision == True:
            return  out_2, correction
        else:
            return  out_2
